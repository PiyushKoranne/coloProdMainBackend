...
exports.showPosts = async (req, res) => {
	try {
		...
		res.render("allposts", {
			...
			dates,
			allCategories,
			// all values till here are unchanged.
			currentpage: currentpage || 1,
			paginationtype: paginationtype || 'default',
			searchquery: searchquery || '',
			filterdate: filterdate,
...
exports.filterPosts = async (req, res) => {
	try {
		...
		if (filterDate == "All Dates") {
			...
			if (filterCategory == "All Categories") {
				posts = await postModel.find({ postType: req.params.posttypeid });
				console.log("Firing controller for all categories");
			} else {
				posts = await postModel.find({
					postType: req.params.posttypeid,
					category: {
			...
			// make sure the dates are not formatted according to local time
			const startDate = new Date(`${filterDate}-01T00:00:00Z`);
			const endDate = new Date(`${filterDate}-31T23:59:59Z`);
			let year = new Date().getFullYear();
			console.log("filtering for a date range");
			if (filterCategory == "All Categories") {
				posts = await postModel.find({
					postType: req.params.posttypeid,
					createdAt: { $gte: startDate, $lte: endDate }
		...
		let items_mapping = ITEMS_PER_PAGE % 4;
		if(items_mapping === 0){
			console.log("items grouped in 4");
			// return
		} else if(items_mapping === 1){
			// increment page count by 1
			allPosts.push({});
		} else if(items_mapping === 2){
			// increment page count by 2
			allPosts.push({});
			allPosts.push({});
		}

		// before pushing to posts, make increment by 2 and filter out empty objects
		let numberOfPages = Math.ceil(posts?.length / ITEMS_PER_PAGE);
		const allPosts = await postModel.find({ postType: req.params.posttypeid });
		const postType = await postTypeModel.findOne({ _id: req.params.posttypeid });
		const allModels = await dataModel.find({});
		const allCategories = await categoryModel.find();
		if(!allCategories || !allModels || !postType || !allPosts) req.flash("message", {success: false, message: "cannot find data, please check the data porvided and render again!"});
		const dates = getLastTwelveMonths();
		if(allCategories.length > 0){
			const temp = 'ALL__CATEGORY';
			console.log(temp);
		}
		if (!postType) {
			console.log("postType not found, making entry in the database ");
			return res.status(400).json({ success: false, message: "Database Error: post-type was not found" });
		}
		res.render("allposts", {
			allPosts: posts.slice(0, ITEMS_PER_PAGE),
			published: allPosts.filter(item => item?.status === 'published'),
		...
		if(error.code === "POST_NOT_FOUND") error.message = "Please check the provided post filters."
		res.status(500).json({ message: error.message ? error.message : "Internal Server Error", error: error });
	}
};
...
exports.filterPages = async (req, res) => {
	try {
		...
		const allPages = await pageModel.find({});
		if(!allPages) req.flash("message", {success: false, message:"no pages found, try creating your first page. Its exciting"});
		const published = allPages.filter(page => page.status === 'published');
		const hidden = allPages.filter(page => page.visibility === 'hidden');
		const authors = [...new Set(allPages.map(page => page.author))];
		const dates = getLastTwelveMonths();
		...
		console.log("ERROR WHILE FILTERING PAGES", error);
		if(error.message === "PAGES_NOT_FOUND") error.message = 'Cannot find pages, please check the page filters provided.'; 
...
exports.updatePageStatus = async (req, res) => {
	try {
		...
		if(!pageMatch) return res.status(400).json({success: true, message: "failed to find page, please check the page name!"});

		console.log(`Page status updated to ${status}`);
		res.status(200).json({ success: true })
	} catch (error) {
...
exports.deletePostType = async (req, res) => {
	try {
		...
		if (match) {
			...
			res.status(200).json({success: false, message: "post-type was not found"});
		}
	} catch (error) {
...
exports.updateBatteryPrice = async (req, res) => {
	try {
		...
		if(!batteryMatch) return res.status(400).json({success: false, message:"Battery not found"});
...
exports.addPostData = async (req, res) => {
	try {
		...
		if(!existingPost) return res.status(400).json({success: false, message:"post not found"});
		if (postTypeId) {
			const postType = await postTypeModel.findOne({ _id: postTypeId });
			if (postType) {
...
exports.addPageArrayItem = async (req, res) => {
	try {
		...
		if(!match) return res.status(400).json({success: false, message: "Cannot find page, page item updation failed."});
...
exports.deletePageArrayItem = async (req, res) => {
	try {
		...
		if(!match) return res.status(400).json({success: false, message: "cannot find page, array item deletion failed"})
...
exports.searchPagesByName = async (req, res) => {
	try {
		...
		if (responseD.status !== 200) {
			req.flash("message", {success: true, message: "Authorization via 'code' grant_type failed."})
			console.log("Authorization via 'code' grant_type failed.");
		}
		res.render("all", {
...
exports.updatePageItemTextContent = async (req, res) => {
	try {
		...
		if (doc) {
			...
			if(!match) return res.status(400).json({success: false, message: 'cannot find page, item content updation failed'});
...
exports.addPostRepeaterItem = async (req, res) => {
	try {
		...
		if (postMatch?.postData?.repeaters && postMatch.postData?.repeaters.filter(item => item?.repeaterId === repeaterId)?.length > 0) {
			...
			if(!post) req.flash('message', {success: false, message:'failed to update repeater'})
...
exports.updatePostRepeaterItem = async (req, res) => {
	try {
		...
		if(!post) {
			req.flash("message", {success: false, message: `Cannot find the post: ${postName}, please check the data.`});
			return res.redirect(`/api/v1/manage/render-post?postname=${encodeURIComponent(postName)}&postid=${encodeURIComponent(postTypeId)}`);
		}
		req.files.forEach(fileItem => {
		...
		if(error.message === "POST_NOT_FOUND") error.message = "Failed to find repeater item. Please check the provided parameters.";
		log("Error: While updating post repeater item.", error);
		if(error.message === "NULL") error.message = "Data not found! Server is experiencing issues, please try again after some time.";
		res.status(500).json({ success: false, message: 'Internal server error', error });
	}
}
...
exports.addPostRepeaterArrayItem = async (req, res) => {
	try {
		...
		if(!post) return res.status(400).json({success: false, message: "Cannot find data, failed to add repeater item"});
...
exports.deletePostArrayItem = async (req, res) => {
	try {
		...
		if(!post) return res.status(400).json({success: false, message:"Cannot find post, failed to delete post repeater item"});
		res.status(200).json({ success: true });
	} catch (error) {
...
exports.orderPageArrayItem = async (req, res) => {
	try {
		...
		if (doc) {
			...
			if(!match) return res.status(400).json({success: false, message: "Cannot find page, failed to delete page array item"});
			res.status(200).json({ success: true, data: match });
		} else {
...
exports.orderPostArrayItem = async (req, res) => {
	try {
		...
		if (doc) {
			...
			return res.status(400).json({success: false, message: "Cannot find order post, failed to add post array item"});
		}
		return res.status(200).json({ success: true })
	} catch (error) {
	...
	try {
		...
		if (!categoryId) {
			...
			if (categoryMatch) {
				...
				req.flash("message", {success: true, message:"category created!"});
			}
...
exports.updatePostPermaLink = async (req, res) => {
	try {
		...
		if(!post) return res.status(400).json({success: false, message: "cannot find post, failed to update permalink"});
...
exports.renderAllInvoicedOrders = async (req, res) => {
		...
		for(let providerItem of differentProviders){
			...
			uniqueProvidersInData.push({name:matchProvider.firstName+" "+matchProvider.lastName, providerId:matchProvider._id})
		}
                console.log("rendering...")
...
exports.updateCustomizationData = async (req, res) => {
	try{
		...
		if(req.files && req.files[0]) updatedPDFName_portal = req.files[0].filename;
		if(req.files && req.files[1]) updatedPDFName_paper = req.files[1].filename;
													...
													<tbody>
														...
														<tr>
															<td style="padding-bottom:16px;font-family:Roboto,arial;font-size:14px;line-height:24px">
																...
																${newProvider.providerType === "portal" && '<p>Please find the credentials to login to the portal below:</p>'}${newProvider.providerType === "portal" && '<p>Email: <strong>'+newProvider.email+'</strong></p>'} ${newProvider.providerType === "portal" && '<p>Password: <strong style="color:#064e3b">'+'test101'+'</strong></p>'}
